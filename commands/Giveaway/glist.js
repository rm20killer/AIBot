        ...
        if (message.member.roles.cache.find(r => r.id === modid)) {
            if(args[0])
            {
                if (args[0] == "all"){
                    ...
                    .setColor(`#0099ff`)
                    .setDescription(`${allGiveaways.map(giveaway => `**${giveaway.messageId}**: ${giveaway.prize} end*ed* on `+unixToDate(giveaway.endAt)).join('\n')}`)
                    ...
                    .setColor(`#0099ff`)
                    .setDescription(`${notEnded.map(giveaway => `**${giveaway.messageId}**: ${giveaway.prize} end on `+unixToDate(giveaway.endAt)).join('\n')}`)
